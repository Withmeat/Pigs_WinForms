...
{
    public enum size { Small, Medium, Big, NoChange };
    public class pigsModel
    {
        ...
        private (Queue<Keys>, Queue<Keys>) _keyQueue;
        private IFightPigsDataAccess _dataAccess;

        public event EventHandler<pigsEventArgs>? GameAdvanced;

        public (Queue<Keys>, Queue<Keys>) Keys { get { return _keyQueue; } }
        ...
        public void NewGame()
        {
            if (Size != size.NoChange)
            {
                ...
                _currentPlayer = 0;
                _keyQueue = (new Queue<Keys>(), new Queue<Keys>());
            }
        }

        public void KeyInputHandler(Keys keyInput)
        {
            ...
            if(stepsOver) _keyQueue = (new Queue<Keys>(), new Queue<Keys>());
        }
